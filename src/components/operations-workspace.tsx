...
type OperationsData = { tasks: Task[]; maintenance: Maintenance[]; products: Product[]; dailyCloses: Close[]; notes: unknown[] };
...
export function OperationsWorkspace() {
  ...
  async function createTask(formData: FormData) {
    setBusy(true);
    const response = await fetch("/api/v1/operations", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ kind: "task", payload: { title: formData.get("title"), description: formData.get("description") || undefined, priority: formData.get("priority"), dueAt: formData.get("dueAt") ? new Date(String(formData.get("dueAt"))).toISOString() : undefined } }) });
