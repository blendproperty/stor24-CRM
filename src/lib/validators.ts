...
export const moveInSchema = z.object({ reservationId: id.optional(), facilityId: id, customerId: id, unitId: id, startDate: z.coerce.date(), monthlyRate: money.optional(), initialCharge: money.default(0), accessState: z.string().trim().min(1).max(40).default("PENDING"), signerName: z.string().trim().min(2).max(120), initials: z.array(z.enum(LEASE_CLAUSE_KEYS)).refine((value) => LEASE_CLAUSE_KEYS.every((key) => value.includes(key)), "Every clause of the lease agreement must be initialled.") });
