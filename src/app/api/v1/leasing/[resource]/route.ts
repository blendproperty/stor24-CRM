  ...
  try {
    ...
    if (resource === "facilities") { if (!scope.unrestrictedFacilities) throw new Error("FORBIDDEN"); const current = await db.facility.findFirst({ where: { id: body.id, ...facilityWhere(scope) } }); if (!current) throw new Error("NOT_FOUND"); entity = await db.facility.update({ where: { id: current.id }, data }); }
      ...
      if (resource === "units") {
        ...
        if (unitData.unitTypeId && !(await db.unitType.findFirst({ where: { id: unitData.unitTypeId, facilityId: current.facilityId } }))) throw new Error("CONFLICT");
        const linked = await db.unit.findUnique({ where: { id: current.id }, include: { occupancies: { where: { status: "ACTIVE" } }, reservations: { where: { status: "ACTIVE" } } } });
        if (unitData.status && (linked?.occupancies.length || linked?.reservations.length)) throw new Error("CONFLICT");
      }
      entity = await (model as typeof db.unit).update({ where: { id: current.id }, data }); }
...
export async function DELETE(request: Request, context: { params: Promise<{ resource: string }> }) {
  try {
    ...
    if (resource === "customers") await requirePermission("operations.manage");
    if (["facilities", "unit-types", "units"].includes(resource)) await requirePermission("inventory.manage");
    const requestUrl = new URL(request.url); const id = requestUrl.searchParams.get("id"); const force = requestUrl.searchParams.get("force") === "true"; if (!id) throw new Error("NOT_FOUND"); const scope = await requireScope();
    ...
    else { const model = resource === "unit-types" ? db.unitType : resource === "units" ? db.unit : resource === "leads" ? db.lead : db.reservation; const entity = await (model as typeof db.unit).findFirst({ where: { id } }) as { facilityId: string } | null; if (!entity) throw new Error("NOT_FOUND"); await requireFacility(scope, entity.facilityId); if (resource === "units") await db.unit.update({ where: { id }, data: { status: "UNAVAILABLE" } }); else if (resource === "reservations") { await db.reservation.update({ where: { id }, data: { status: "CANCELLED" } }); } else { if (resource === "unit-types") { const assigned = await db.unit.count({ where: { unitTypeId: id } }); if (assigned && !force) return Response.json({ error: { code: "UNIT_TYPE_IN_USE", message: `This unit type is used by ${assigned} unit${assigned === 1 ? "" : "s"}. An organisation owner can delete the type and its unused units.`, assigned, canForceDelete: true } }, { status: 409 }); if (assigned && force) { await requireOwner(); const protectedUnits = await db.unit.count({ where: { unitTypeId: id, OR: [{ reservations: { some: {} } }, { occupancies: { some: {} } }] } }); if (protectedUnits) return Response.json({ error: { code: "UNIT_TYPE_HAS_HISTORY", message: `${protectedUnits} assigned unit${protectedUnits === 1 ? " has" : "s have"} reservation or occupancy history and cannot be deleted. Reassign those units first.` } }, { status: 409 }); await db.$transaction([db.lead.updateMany({ where: { desiredUnitTypeId: id }, data: { desiredUnitTypeId: null } }), db.maintenanceRequest.updateMany({ where: { unit: { unitTypeId: id } }, data: { unitId: null } }), db.unit.deleteMany({ where: { unitTypeId: id } }), db.unitType.delete({ where: { id } })]); } else await (model as typeof db.unit).delete({ where: { id } }); } else await (model as typeof db.unit).delete({ where: { id } }); } }
