...
export async function GET() {
  try {
    ...
    const accounts = await db.account.findMany({
      ...
      include: { customer: true, tenancy: { include: { facility: true, occupancies: { where: { status: { in: ["ACTIVE", "NOTICE_GIVEN"] } }, include: { unit: { include: { unitType: true } } } } } }, ledgerEntries: { orderBy: [{ effectiveAt: "desc" }, { createdAt: "desc" }], take: 50 }, payments: { orderBy: { createdAt: "desc" }, take: 25 } },
