...
- Reservation cancellation (`cancelReservation()` in `src/lib/leasing-service.ts`, `DELETE /api/v1/reservations?id=<reservationId>`) is a real, first-class endpoint — not part of the generic `/api/v1/leasing/workflows/[action]` route. It requires `reservations.manage` permission scoped to the reservation's facility, only allows cancelling a reservation currently in `ACTIVE` status, sets it to `CANCELLED` in a transaction, and releases the unit back to `AVAILABLE` if no other `ACTIVE` reservation or occupancy still references it. The staff UI is on the "Reservations & holds" screen (`src/components/reservations-workspace.tsx`): each `ACTIVE` reservation row has a red "Cancel" button that shows a confirm dialog ("Cancel the reservation for unit {number} and release the unit?") before calling the delete endpoint, then shows a success banner. Reservations are also created directly from this screen via "New reservation" (`POST /api/v1/reservations` → `createReservation()`), separate from the public booking path.
...
- **Reservation-cancel path has not yet been live-tested.** The `cancelReservation()` service logic exists and appears correct (validates `ACTIVE` status, sets `CANCELLED`, releases the unit to `AVAILABLE` if no other holds), but has not been exercised against a live reservation in production. See "How to prove the reservation cancel path live" below for the exact manual test procedure.
...
## How to prove the reservation cancel path live

Confirmed 18 August 2026 by reading `src/app/api/v1/reservations/route.ts`, `src/lib/leasing-service.ts` (`cancelReservation`, `createReservation`) and `src/components/reservations-workspace.tsx`. This requires an explicit, deliberate run by a staff user with `reservations.manage` permission — it mutates real production data — so it is written here as a manual step-by-step for Brett (or whoever runs it), not something an AI assistant should trigger unattended against production.

1. **Pick a throwaway test unit.** Use a Midpoint unit that is currently genuinely `AVAILABLE` and not one you care about keeping clean in reporting — e.g. re-use the same pattern as the two existing live test reservations (`John Wayne` / unit 103, `Blend Group` / unit 360) or create a new obviously-fake customer name like "Test Cancel — do not lease".
2. **Create the reservation** either:
   - via the public site (`/book` or `/storage/midpoint`) exactly like a real customer would, or
   - directly in the CRM: go to the **Reservations & holds** screen, click **New reservation**, pick the test unit (must currently show `AVAILABLE`) and submit.
3. **Confirm it landed correctly:** on Reservations & holds, the new row should show status `ACTIVE`, the correct unit, quoted rate and hold-expiry. On the unit inventory / move-in screens, that unit should now show as `Reserved` (amber), not `Vacant`.
4. **Cancel it:** on the same Reservations & holds row, click the red **Cancel** button. A confirm dialog appears — *"Cancel the reservation for unit {number} and release the unit?"* — confirm it.
5. **Confirm the result:** a success banner should read *"Reservation cancelled. Unit {number} is available again."* The reservation row should now show status `CANCELLED` (or disappear from the active list, depending on the screen's filtering). Separately, check the unit inventory / move-in screen again — the unit should be back to `Vacant`/`AVAILABLE`.
6. **Optional but recommended for the audit trail:** screenshot steps 3 and 5 (before/after unit status, and the cancelled reservation row) so there's durable evidence beyond "I clicked it and it seemed to work" — the same standard already applied to the two live reserve-side tests on 17 August.
7. Report back (or note here) the exact unit number/reservation used and the timestamp, so this file can be updated with real evidence instead of "not yet tested."

If preferred, this can instead be run by an AI assistant with live, authenticated browser access to the CRM (not currently connected in this environment) — in that case the same seven steps apply, just executed via the browser tools instead of manually, and still require the same explicit go-ahead per the "explicit approval" rule already in this file's Working rules.

## Remaining sub-blockers for "complete core customer-to-reservation-to-lease lifecycle" — confirmed 18 August 2026

`moveIn()` itself is code-complete (see Implemented foundations), but four downstream pieces of the full customer → reservation → lease journey are still open, confirmed against the current codebase:

1. **Payment capture at move-in.** `moveIn()` posts an `initialCharge` as a `LedgerEntry`, but there is no South African payment/debit-order provider wired in to actually collect that charge from the customer. This is the same hard blocker already tracked under Pilot facility and first-release scope and Partially built above — provider selection has not happened yet.
2. **Lease document / e-signature.** Nothing in `moveIn()` generates or captures a signed lease agreement. An e-signature provider has not been selected (tracked as its own Asana item, "Implement e-signature and document workflow").
...
4. **Recurring billing tie-in.** The monthly billing cron is now live and reachable in production (see Implemented foundations), but it has never run against a tenancy created by a real `moveIn()` — the two features (occupancy creation → recurring charge generation) need to be proven together, not just individually.

None of these four are things an AI assistant can complete unilaterally — each needs either a business decision (provider selection), a paid/credentialed external service, or an explicit live test against production. They are the concrete definition of "not yet done" for Task 4 in the roadmap; treat this list, not a vague "lifecycle incomplete" status, as the actionable breakdown.
...
- **First-release scope:** full self-serve — a customer can browse, book, pay and receive working access, end to end, for the Midpoint facility. Concretely this means the release is not "done" until it includes:
  ...
  2. A live, provable reserve/cancel booking lifecycle for Midpoint (Task 3 — the reserve side is live-tested; cancel is not yet tested, see "How to prove the reservation cancel path live" above; email notification is now proven live, SMS/WhatsApp are not yet, see Current status and evidence limits).
...
2. With explicit approval, prove the reservation cancel path — follow "How to prove the reservation cancel path live" above (reserve side is already live-tested — see Current status and evidence limits).
...
7. Select and implement payment and Hikvision access providers — both are hard blockers for the approved pilot scope (see Pilot facility and first-release scope above) — plus e-signature and insurance providers through the existing provider boundaries. See "Remaining sub-blockers" above for how these map onto Task 4.
