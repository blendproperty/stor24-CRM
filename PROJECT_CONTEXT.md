...
- **Task 5 debit-order UAT follow-ups, 21 August 2026.** Unit 103 is the active fresh auto-countersign test. Debit-order contact person now defaults to the Storer name for both individuals and companies but remains signer-editable when the company/different contact requires another value. Initial move-in charge must later default from the selected unit price (then facility default), be locked for ordinary staff, allow Facility Manager/Owner/Admin override only, and audit original value, replacement value, actor, timestamp and reason.
...
8. Run the debit-order Task 5 send→sign→activate journey using v4/52 fields, visually confirm required/optional styling on the fresh signer form, and prove the same activation/link/idempotency invariants. After UAT, add completed-PDF/certificate retrieval plus retry/reconciliation UI.
