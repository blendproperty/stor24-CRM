...
- **BlendSign signer-experience and optional authorised countersigning subtask, 21 August 2026 — code-complete locally, not yet live.** Stor24 now sends only `tenant.phone` (the duplicate `tenant.telephone` merge value is removed). The final template PDFs use one Mobile number row. BlendSign provides city selection/postal autofill and clearer `Signed at`/final-execution guidance. Facility Program Defaults > Move In now has a separate `blendSignAutoCountersign` control; when true, only the Stor24 Rep recipient is flagged for BlendSign auto-signing. BlendSign must independently have an authorised representative, signature, initials and company permission enabled, otherwise it rejects the auto-sign request. Leave the Stor24 control off for manual UAT. Local evidence: all 40 tests, typecheck and the full Next.js production build pass.
...
8. Deploy the BlendSign/Stor24 signer-experience and authorised-countersigning changes, publish the final single-mobile PDFs/field definitions, then re-run the disposable non-debit send→sign→activate journey with auto-signing off to close Task 4. Only then run the debit-order Task 5 journey. After UAT, add completed-PDF/certificate retrieval plus retry/reconciliation UI.
