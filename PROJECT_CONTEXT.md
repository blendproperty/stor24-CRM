...
- **Lease e-signature capture (in-house), 18 August 2026 — code-complete, not yet migrated/deployed/live-tested.** Per Brett's decision to build e-signature in-house (simple e-signature + audit trail, blocking step inside move-in) and his follow-up instruction that there must be an actual agreement initialled and signed by the tenant, not just a typed name: the lease is now built as a **structured list of clauses** (`src/lib/lease-agreement-content.ts` — premises & use, term & rent, access & security, insurance & liability, prohibited items, default & termination, data & privacy). `moveIn()` requires a typed `signerName` and an `initials` array that must cover every clause key (enforced by `moveInSchema`, `422 VALIDATION_ERROR` otherwise); the move-in UI renders every clause with its own required "I have read and initial this clause" checkbox before the final typed-name signature. The full rendered document is hashed (SHA-256) and stored, along with per-clause initials (`{clauseKey, initialedAt}` JSON), the clause-text version (`clauseVersion`), signer name, and server-captured IP/user-agent, in a `Document` row (`type: "LEASE_AGREEMENT"`) inside the same transaction as tenancy creation. `Document` was extended across two additive migrations: `prisma/migrations/20260818120000_lease_esignature` (`content`, `sha256`, `signerName`, `signerIp`, `signerUserAgent`) and `prisma/migrations/20260818130000_lease_clause_initials` (`initials`, `clauseVersion`). Full design in `docs/ESIGNATURE_DESIGN.md`. **Not yet done:** neither migration has been run against the production database (`npm run db:migrate:deploy` on the VPS), the code has not been deployed, and no live move-in has been exercised through this path yet — do not report this as "live" until those three things happen. **Remaining real gap, not just a fast-follow:** the clause wording in `lease-agreement-content.ts` is still drafted-but-not-approved boilerplate — Brett or STOR 24's attorney must review and approve the actual clause text before this is relied on for real tenants (bump `LEASE_VERSION` once approved). PDF rendering/durable storage is a separate, lower-priority fast-follow (content currently lives inline in Postgres).
...
- **E-signature — decision made 18 August 2026: build in-house rather than select a third-party e-signature vendor (e.g. DocuSign/Adobe Sign). Structured, per-clause-initialled version is now code-complete (see Implemented foundations and `docs/ESIGNATURE_DESIGN.md`) but not yet migrated, deployed or live-tested — and the clause wording itself is still unreviewed draft text.** Remaining before this can be called done: (1) run both new Prisma migrations against the production database, (2) deploy, (3) complete one real move-in through the signed-lease path with the `Document` row inspected afterward to confirm content/hash/signer/initials fields are populated as expected, and (4) get Brett or an attorney to review and approve the actual clause wording in `src/lib/lease-agreement-content.ts` before relying on this for real tenants.
...
- `main` includes the facility-patch-validation fix (PR #8), the public-booking-setup persistence fix (PR #7), the company-setup-inputs fix (PR #6), the public-booking API feature (PR #5), the SendGrid email path, and the recurring monthly billing feature (`billing-service.ts`, `/api/v1/billing/run-monthly`, real `/billing` and `/collections` screens) — all confirmed merged and deployed as of 18 August 2026. The lease e-signature changes (schema, service, validators, API route, server action, UI, structured clause content) are merged to `main` as of 18 August 2026 but **not yet deployed or migrated against production** — see Partially built above.
...
2. **Lease document / e-signature.** **Status: structured, per-clause-initialled version built.** `moveIn()` now requires the customer to initial every clause of the lease agreement individually plus a final typed full-name signature; the full document is rendered, hashed and stored (with per-clause initials, clause version and a signer/IP/UA audit trail) in a `Document` row inside the move-in transaction — see Implemented foundations and `docs/ESIGNATURE_DESIGN.md`. Not yet migrated against production, not yet deployed, not yet live-tested. The clause text itself is still draft boilerplate pending Brett/attorney review — this is a real gap, not just a fast-follow, and must be closed before real tenants sign.
...
Items 1 and 3 need a business/provider decision or credentialed external service before they can be built at all. Item 2 has moved from "needs scoping" to "structured per-clause code merged, needs deploy + migration + live test + legal review of the wording." Item 4 is the closest to fully done — it just needs one more observed billing run.
...
- **New Prisma migrations must be deployed with `npm run db:migrate:deploy` on the VPS before code that depends on them is live-tested.** Two lease e-signature migrations — `20260818120000_lease_esignature` and `20260818130000_lease_clause_initials` (both additive `Document` columns) — are merged to `main` but have not yet been run against production as of 18 August 2026. Deploying the app code without running both migrations first would cause `moveIn()` to fail at runtime on the new `Document` columns.
...
7. Select and implement a South African payment provider (in talks as of 18 August 2026) and a Hikvision access provider — both hard blockers for the approved pilot scope. **E-signature is now built as a structured, per-clause-initialled agreement (18 August 2026, see `docs/ESIGNATURE_DESIGN.md`)** — remaining: run both new migrations on the VPS, deploy, live-test one full move-in through the signed-lease path, and get Brett/an attorney to review and approve the actual clause text in `src/lib/lease-agreement-content.ts` before it's relied on for real tenancies. Also select an insurance provider through the existing provider boundaries.
