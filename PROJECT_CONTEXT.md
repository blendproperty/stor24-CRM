...
- **BlendSign lease routing, added 21 August 2026.** The staff move-in form requires a payment method. `DEBIT_ORDER` routes to `stor24-unit-lease-debit-order`; card, EFT and other route to `stor24-unit-lease`. A server-only client creates an idempotent BlendSign envelope, maps known customer/unit/lease values, stores the external envelope reference on `Document`, and the signed `envelope.completed` webhook activates the pending occupancy and tenancy. **Code-complete, pushed on `codex/blendsign-lease-routing` at `15f9047`, and locally verified by typecheck, production build and 39 passing tests; not merged, deployed, configured or live-tested.** Exact live template identifiers, merge keys and the `Signer 1` → `Stor24 Rep` order were checked in BlendSign. The legacy `/sign/[token]` flow remains for existing records. Completed-PDF/certificate retrieval and retry/reconciliation UI remain outstanding.
...
8. Deploy and configure the two-repository BlendSign lease integration in dependency order: BlendSign first (backup, migration, app/worker, health/API proof), then Stor24 portal (migration/app). Configure the server-only API key and signed webhook secret/URL. Run disposable debit-order and non-debit send→customer-sign→representative-sign→webhook→activate journeys; prove template selection, field population, signer order, external document reference and idempotent retry. Negative-test invalid webhook signatures and provider failure. Then add completed-PDF/certificate retrieval plus Documents and retry/reconciliation UI.
