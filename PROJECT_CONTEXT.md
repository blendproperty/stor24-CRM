...
- **E-signature — decision made 18 August 2026: build in-house rather than select a third-party e-signature vendor (e.g. DocuSign/Adobe Sign).** This reverses the earlier assumption of buying a provider (see "Implement e-signature and document workflow" in Asana). Nothing has been scoped or built yet — see Priority next work for the open scoping questions (signature capture method, evidentiary/compliance bar under South Africa's ECT Act, storage of the signed artefact, audit trail) that need answers before implementation starts.
...
- **Reservation-to-tenancy move-in engine (`moveIn()`): live evidence found 18 August 2026, see Implemented foundations.** The Blend Group/unit 360 reservation is `CONVERTED` with a real `ACTIVE` tenancy and account already in production. This closes the main remaining gap on "complete core customer-to-reservation-to-lease lifecycle" for the engine itself — the four items in "Remaining sub-blockers" below (payment capture, e-signature, access provisioning, and confirming the billing tie-in against this real tenancy) are what's left, not the move-in engine itself.
...
`moveIn()` itself is code-complete and now has live evidence of working correctly (see Implemented foundations — the Blend Group/unit 360 conversion). Four downstream pieces of the full customer → reservation → lease journey are still open:

1. **Payment capture at move-in.** `moveIn()` posts an `initialCharge` as a `LedgerEntry`, but there is no South African payment/debit-order provider wired in to actually collect that charge from the customer. **Status 18 August 2026: Brett is now in talks with providers** — not yet selected or contracted. Still tracked under Pilot facility and first-release scope and Partially built above.
2. **Lease document / e-signature.** Nothing in `moveIn()` generates or captures a signed lease agreement. **Decision made 18 August 2026: build this in-house rather than buy a third-party e-signature provider.** Needs scoping before implementation starts — see Priority next work.
...
Items 1 and 3 need a business/provider decision or credentialed external service before they can be built at all. Item 2 has just had its build-vs-buy decision made and now needs scoping. Item 4 is the closest to done — it just needs one more observed billing run.
...
7. Select and implement a South African payment provider (in talks as of 18 August 2026) and a Hikvision access provider — both hard blockers for the approved pilot scope. Scope and build the in-house e-signature capability (decision made 18 August 2026, not yet scoped) — open questions: signature capture method (typed/drawn/OTP-confirmed), evidentiary standard needed under South Africa's Electronic Communications and Transactions (ECT) Act (a "simple" electronic signature vs. an "advanced" one requiring an accredited authentication product), where the signed PDF/artefact and an audit trail (IP, timestamp, document hash) get stored, and whether it needs to integrate into the `moveIn()` transaction or can be a separate pre-move-in step. Also select an insurance provider through the existing provider boundaries.
