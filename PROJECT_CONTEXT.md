...
- **Reservation-to-tenancy lifecycle (`src/lib/leasing-service.ts`) is real and database-backed, not a stub — and now live-tested, not just code-reviewed.** `createReservation` claims a unit and moves its lead to `RESERVED`. `moveIn` runs a single transaction that creates the `Account`, an `ACTIVE` `Tenancy` with its first `Occupancy`, sets the unit `OCCUPIED`, posts an optional move-in `LedgerEntry`, captures a signed, per-clause-initialled lease `Document` (see below), and — when started from a reservation — marks that reservation `CONVERTED`. `transfer`, `giveNotice` and `moveOut` cover the rest of the tenant lifecycle, all scoped and audited. Wired to real routes (`/api/v1/leasing/workflows`) and real staff screens (Reservations, Customers & tenants, Move in) — not the demo-mock pattern `/communications` still has. **Live evidence found 18 August 2026:** querying `/api/v1/reservations` and `/api/v1/accounts` in production shows the original `Blend Group` / unit 360 reservation (`cmssw0z0f...`) is now `CONVERTED`, with `convertedTenancyId` pointing at a real `ACTIVE` `Tenancy` (`cmsy5s5pm...`) under a real `Account` (`ST24-MSY5S5OA`, balance `-0.01` reflecting an initial-charge ledger entry). This confirms the Tenancy/Occupancy/Account chain and the reservation `CONVERTED` flip do work correctly in production — this was not run by an AI assistant this session; it was already done live before this check (exact operator/timestamp not captured, `tenancy.updatedAt` reads 2026-08-18T04:24:37Z). This predates the lease e-signature capture described below, so this specific historical tenancy has no signed `Document`. Downstream legs (payment capture on the `initialCharge`, Hikvision access activation from `accessState`) are still not implemented — see Partially built below.
- **Lease e-signature capture (in-house), 18 August 2026 — built, deployed, and live-tested end to end.** Per Brett's decision to build e-signature in-house (simple e-signature + audit trail, blocking step inside move-in) and his follow-up instruction that there must be an actual agreement initialled and signed by the tenant, not just a typed name: the lease is a **structured list of 7 clauses** (`src/lib/lease-agreement-content.ts` — premises & use, term & rent, access & security, insurance & liability, prohibited items, default & termination, data & privacy). `moveIn()` requires a typed `signerName` and an `initials` array covering every clause key (enforced by `moveInSchema`, `422 VALIDATION_ERROR` otherwise); the move-in UI renders every clause with its own required "I have read and initial this clause" checkbox before the final typed-name signature, and disables "Complete move-in" client-side until all are ticked. The full rendered document is hashed (SHA-256) and stored, along with per-clause initials (`{clauseKey, initialedAt}` JSON), the clause-text version (`clauseVersion`), signer name, and server-captured IP/user-agent, in a `Document` row (`type: "LEASE_AGREEMENT"`) inside the same transaction as tenancy creation. `Document` was extended across two additive migrations: `prisma/migrations/20260818120000_lease_esignature` and `prisma/migrations/20260818130000_lease_clause_initials`. Full design in `docs/ESIGNATURE_DESIGN.md`. **Deployed automatically 18 August 2026** via the repo's existing `Deploy to VPS` GitHub Actions workflow (triggers on `workflow_run` after `CI` passes on `main`; SSHes into the VPS using its own stored secrets and runs `docker compose run --rm migrate` then rebuilds/restarts the app — no manual server access was used or needed). **Live-tested 18 August 2026:** at `/operations/move-in`, selected vacant unit 145 (FF-F13), customer John Wayne, R100/month, initialled all 7 clauses individually (each correctly rendered with the real facility/unit/customer/rate substituted in), typed "John Wayne" as signature. Submit stayed disabled until all 7 were ticked. Submitted successfully, redirected to `/operations/accounts`. Verified via `/api/v1/leasing`: unit 145 flipped to `OCCUPIED`, a real `ACTIVE` tenancy exists for John Wayne under account `ST24-MSYH8P3N`. Because the `Document` row is created inside the same transaction as the tenancy, the successful tenancy creation confirms the signed document write also succeeded. **Remaining real gap, not just a fast-follow:** the clause wording in `lease-agreement-content.ts` is still drafted-but-not-approved boilerplate — Brett or STOR 24's attorney must review and approve the actual clause text before this is relied on for real tenants (bump `LEASE_VERSION` once approved). PDF rendering/durable storage is a separate, lower-priority fast-follow (content currently lives inline in Postgres).
...
- **E-signature — decision made 18 August 2026: build in-house rather than select a third-party e-signature vendor (e.g. DocuSign/Adobe Sign). Structured, per-clause-initialled version is deployed and live-tested (see Implemented foundations and `docs/ESIGNATURE_DESIGN.md`) — the mechanism itself is proven working end to end.** The one thing still outstanding: the clause wording in `src/lib/lease-agreement-content.ts` is still drafted-but-not-approved boilerplate — Brett or an attorney must review and approve the actual clause text (then bump `LEASE_VERSION`) before this is relied on for real tenants.
...
- `main` includes the facility-patch-validation fix (PR #8), the public-booking-setup persistence fix (PR #7), the company-setup-inputs fix (PR #6), the public-booking API feature (PR #5), the SendGrid email path, and the recurring monthly billing feature (`billing-service.ts`, `/api/v1/billing/run-monthly`, real `/billing` and `/collections` screens) — all confirmed merged and deployed as of 18 August 2026. The lease e-signature changes (schema, service, validators, API route, server action, UI, structured clause content) are merged, deployed and live-tested as of 18 August 2026 — see Implemented foundations.
...
- **Reservation-to-tenancy move-in engine (`moveIn()`): live evidence found 18 August 2026, see Implemented foundations.** The Blend Group/unit 360 reservation is `CONVERTED` with a real `ACTIVE` tenancy and account already in production. This closes the main remaining gap on "complete core customer-to-reservation-to-lease lifecycle" for the engine itself — of the four items in "Remaining sub-blockers" below, lease e-signature is now live-tested (wording still not attorney-approved); payment capture, access provisioning, and confirming the billing tie-in against this real tenancy remain open.
- **Lease e-signature: live-tested end to end 18 August 2026** — see Implemented foundations and "How the lease e-signature was proven live" below for the full walkthrough (unit 145, John Wayne, 7/7 clauses initialled, real tenancy `ACTIVE` under account `ST24-MSYH8P3N`).
...
## How the lease e-signature was proven live (18 August 2026)
...
6. Verified via `GET /api/v1/leasing`: unit 145's status is `OCCUPIED`, and a tenancy exists with `status: "ACTIVE"`, `customer: "John Wayne"`, `account.accountNumber: "ST24-MSYH8P3N"`. Because `moveIn()` creates the `Document` row (content, sha256, signerName, signerIp, signerUserAgent, initials, clauseVersion) inside the same database transaction as the tenancy, this successful tenancy creation is sufficient evidence the signed document write also committed — a failure there would have rolled back the whole transaction and the tenancy would not exist.

This is the first live-tested move-in run through the new e-signature path. The clause wording itself remains unreviewed draft text — see Partially built above.
...
2. **Lease document / e-signature.** **Status: live-tested end to end.** `moveIn()` requires the customer to initial every clause of the lease agreement individually plus a final typed full-name signature; the full document is rendered, hashed and stored (with per-clause initials, clause version and a signer/IP/UA audit trail) in a `Document` row inside the move-in transaction. Deployed via the automatic CI/CD pipeline and live-tested 18 August 2026 (unit 145, John Wayne — see "How the lease e-signature was proven live" above). The one remaining gap: the clause text itself is still draft boilerplate pending Brett/attorney review — must be closed before real tenants sign.
...
Items 1 and 3 need a business/provider decision or credentialed external service before they can be built at all. Item 2 is functionally done — live-tested — with only the legal-review-of-wording step remaining. Item 4 is the closest to fully done on the technical side — it just needs one more observed billing run.
...
7. Select and implement a South African payment provider (in talks as of 18 August 2026) and a Hikvision access provider — both hard blockers for the approved pilot scope. ~~Deploy and live-test the structured e-signature agreement~~ — **done 18 August 2026, see "How the lease e-signature was proven live" above.** Remaining on e-signature: get Brett/an attorney to review and approve the actual clause text in `src/lib/lease-agreement-content.ts` before it's relied on for real tenancies. Also select an insurance provider through the existing provider boundaries.
...
16. **When a user says a signature/agreement mechanism is "not real" or "not binding," treat that as a data-model and UX gap, not a copy tweak.** On 18 August 2026, Brett correctly flagged a typed-name-only signature as insufficient for a real agreement; the fix required a structured clause list, per-clause initial capture, and a genuine signer-facing document view — not just relabelling a checkbox.
